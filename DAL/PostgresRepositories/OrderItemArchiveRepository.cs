...
namespace DAL.PostgresRepositories
{
    internal class OrderItemArchiveRepository : IOrderItemArchiveRepository
    {
        ...
        public OrderItemArchiveRepository(string connectionString)
        {
        ...
        public void Add(OrderItem entity)
        {
            ...
            {
                connection.Open();
                // задаем total_dish_price так как нету триггера для расчета этого поля для этой таблицы (есть только в orders_items)
                var query = "INSERT INTO orders_items_archive (order_id, dish_id, quantity, curr_dish_price, total_dish_price) " +
                    "values (@order_id, @dish_id, @quantity, @curr_dish_price, @total_dish_price) RETURNING id, total_dish_price";
        ...
        public void Delete(OrderItem entity)
        {
            ...
            {
                connection.Open();
                var query = "DELETE FROM orders_items_archive WHERE id = @id";
        ...
        public IEnumerable<OrderItem> GetAll()
        {
            ...
            {
                connection.Open();
                var query = "SELECT * FROM orders_items_archive";
                using (var command = new NpgsqlCommand(query, connection))
                {
                    using (var reader = command.ExecuteReader())
                    {
        ...
        public void Update(OrderItem entity)
        {
            ...
            {
                connection.Open();
                var query = "UPDATE orders_items_archive SET " +
                            "order_id = @order_id, " +
                            "dish_id = @dish_id, " +
                            "quantity = @quantity, " +
